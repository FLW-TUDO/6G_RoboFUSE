...
def read_parse_data(data_parser):
    ...
    try:
        while not data_queue.empty():
            ...
            if dataOk and data_parser.read_serial_dataconfigParameters["detectedObjects"] and len(detObj["x"]) > 0:
                x = detObj["x"]
                y = detObj["y"]
                z = detObj["z"]
                snr = detObj["snr"]
...
def main():
    ...
    try:
        ...
        CLIport, Dataport = serialConfread_serial_dataig(configFileName, controlPort_, dataPort_)
